...
namespace donpablonow\Branding;

// Define constants
define('donpablonow_BRANDING_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('donpablonow_BRANDING_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Load plugin textdomain.
 */
add_action('plugins_loaded', function () {
    load_muplugin_textdomain('donpablonow-custom-branding', 'languages/');
});
...
function login_logo()
{
    ...
    <style type="text/css">
        ...
        body.login div#login h1 a {
            background-image: url( <?=(donpablonow_BRANDING_PLUGIN_URL.'assets/images/logo.svg'); ?> );
            background-repeat: no-repeat;
            background-size: contain;
            width: 300px;
        }

        body.login .donpablonow-info {
            margin-top: 40px;
            text-align: center;
            color: #d0cfcf;
        }

        body.login .donpablonow-info a {
            color: #fff;
            text-decoration: none;
        }

        body.login .donpablonow-info a:hover {
            color: #00a0d2;
        }
...
function create_menu()
{
    ...
    $wp_admin_bar->add_node([
        'id' => $menu_id,
        'title' =>
            '<span class="ab-icon">' . file_get_contents(donpablonow_BRANDING_PLUGIN_DIR . "assets/images/logo.svg") . '</span>',
        'href' => '/'
    ]);
}
